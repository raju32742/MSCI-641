    ...
    for act_name, activation in activations.items():
        ...
        for dropout_rate in dropout_rates:
            for l2_lambda in l2_lambdas:
                print(f'Training with {act_name} activation, hidden size: {hidden_size}, dropout rate: {dropout_rate}, L2 lambda: {l2_lambda}')
                ...
                optimizer = optim.Adam(model.parameters(), lr=initial_lr, weight_decay=l2_lambda)
                scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, 'min', patience=2, factor=0.5, verbose=True)

                # Train the model
                model = train(model, train_loader, val_loader, criterion, optimizer, scheduler, num_epochs)
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Usage: python3 a4/main1.py a1/data <Path to output_dir>")
        sys.exit(1)
